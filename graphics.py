...
# ---------------------------------------------------------------------------
# Robust paths (STRICT: only provided assets), no fallbacks to system fonts
# ---------------------------------------------------------------------------
HERE = os.path.dirname(__file__)

# Кандидаты базовых корней проекта (чтобы покрыть /var/task, локально и т.п.)
_CANDIDATE_BASES = [
    HERE,                                 # .../api
...
def _find_asset(rel_path: str) -> Optional[str]:
    """
    Ищем asset по относительному пути в нескольких базах.
    Пример rel_path: 'fonts/Exo2-Bold.ttf' или 'assets/icons/star.png'
    """
    candidates = []
    for base in _CANDIDATE_BASES:
        candidates.append(os.path.join(base, rel_path))
        # специфичные случаи: иногда пакуют без 'api'
        if rel_path.startswith("fonts/"):
            candidates.append(os.path.join(base, "api", rel_path))
            candidates.append(os.path.join(base, "assets", rel_path[6:]))  # assets/fonts/...
        if rel_path.startswith("assets/"):
            candidates.append(os.path.join(base, "api", rel_path))
    return _first_existing(candidates)

def _must_font(rel_path: str, size: int) -> ImageFont.FreeTypeFont:
    # ищем строго в наших путях
    p = _find_asset(rel_path) or _find_asset(os.path.join("api", rel_path))
    if not p:
        raise FileNotFoundError(f"Font not found via candidates: {rel_path}")
    return ImageFont.truetype(p, size=size)

def _font_exo_b(size: int):      # Имя игрока
    return _must_font("fonts/Exo2-Bold.ttf", size)

def _font_mont_b(size: int):     # Цифры
    return _must_font("fonts/Montserrat-Bold.ttf", size)

def _font_mont_sb(size: int):    # Лейблы/правый текст
    return _must_font("fonts/Montserrat-SemiBold.ttf", size)

def _load_icon(name: str, size: int) -> Optional[Image.Image]:
    p = _find_asset(os.path.join("assets", "icons", name))
    if not p:
        return None
...
BAR_H = 176     # тоньше и ниже
PADDING = 28

# Фиксированные градиенты (не зависят от команды)
GRAD_ORANGE   = ("#FF8A00", "#FFC437")   # card / cards
GRAD_DUO_LEFT = ("#4A1E7C", "#2E135C")   # card2 (левая)
GRAD_DUO_RIGHT= ("#164C91", "#0B356C")   # card2 (правая)
...
def _circle_crop_bottom(img: Image.Image, diam: int) -> Image.Image:
    """
    Круглая вырезка с «якорем» по нижней кромке, увеличено так,
    чтобы макушка не резалась.
    """
    target_h = int(diam * 1.30)
...
def render_card(
    ...
    **kwargs
    ...
    # Логотип — круг меньше, сам логотип крупнее
...
# ---------------------------------------------------------------------------
# Duo card (без зазора, фикс. градиенты)
# ---------------------------------------------------------------------------
def render_card2(
...
# ---------------------------------------------------------------------------
# Special card (правая колонка + звезда без круга)
# ---------------------------------------------------------------------------
def render_card_special(
    ...
    # Правая колонка: звезда без круга + текст
    star = _load_icon("star.png", 36)
...
# ---------------------------------------------------------------------------
# BAD card (какашка без круга, логотип у игрока)
# ---------------------------------------------------------------------------
def render_card_bad(
